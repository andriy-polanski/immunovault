...
angular
  ...
  .factory('wijetsService', ['$window', function($window) {
    ...
    $window.$.wijets.registerAction( {
        ...
        onInit: function () {
            var headerStyle = angular.element(this).getWidgetState('headerStyle');
            if (headerStyle) {
                var widget = angular.element(this).closest('[data-widget]');
            ...
            angular.element(this).find('.dropdown-menu').bind('click', function (e) {
                e.stopPropagation();
            });
            angular.element(this).find('li span').bind('click', function (e) {
                var widget = button.closest('[data-widget]');
                widget.removeClass('panel-info panel-primary panel-blue panel-indigo panel-deeppurple panel-purple panel-pink panel-danger panel-teal panel-green panel-success panel-lime panel-yellow panel-warning panel-orange panel-deeporange panel-midnightblue panel-bluegray panel-bluegraylight panel-black panel-gray panel-default panel-white panel-brown')
                    .addClass(angular.element(this).attr('data-style'));
                angular.element(button).setWidgetState('headerStyle', angular.element(this).attr('data-style'));
                e.stopPropagation();
            });
        }
    });
    ...
    $window.$.wijets.registerAction( {
      ...
      onClick: function () {
        var params = angular.element(this).data('actionParameters');
        var widget = angular.element(this).closest('[data-widget]');
